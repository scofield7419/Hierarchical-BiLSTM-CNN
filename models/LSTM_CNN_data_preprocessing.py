...
def process_file(pd_ok, word_to_id, max_length=600, gb_len=50):
    ...
    y_train_labels = mb.fit_transform(y_train_max)
    ...
    for dx, row in X_train_text.iterrows():
        ...
        contents = handle_text(concat_str)  
    ...
    # return x_pad, X_train_num, y_train
...
def get_data(file_name, embedding_file, save_data_path, maxlen=500, gb_len=50):
    ...
    x_train_texts, y_train = process_file(pd_ok, word_to_id, maxlen, gb_len)
    ####################
    # 划分出一部分为测试集
    print("没有测试集数据，只有训练集……")
    # print("划分测试集数据中……")
    ...
    return words, return_str, (x_train_texts, y_train)
